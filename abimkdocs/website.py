...
-->"""
        ...
        for code, vd in self.variables_code.items():
            vd.write_markdown_files(workdir, comment=do_not_edit_comment)
...
""".format(top.authors)
            ...
            text = front + do_not_edit_comment + """
    ...
    def anchor_from_wikilink(self, token):
        ...
        if namespace is None:
            if name is None:
                ...
                elif name in self.variables_code.external_vars:
                    print("WARNING: got external_vars", name)
                    url, text = "FAKE_URL", name
